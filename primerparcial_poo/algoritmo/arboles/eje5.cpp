...
void borrar_hojas(NAbinario *&arbol,int dato){
NAbinario *anterior=NULL;	
NAbinario *aux=NULL;
nodo_pila *pila=NULL;
if(arbol!=NULL){
...
while(!pilavacia(pila)){
	anterior=aux;
	aux=pila_sacar(pila);
	if(aux->der==NULL && aux->izq==NULL && aux->dato==dato ){
		if(anterior->der->dato==dato){
			anterior->der=NULL;
			delete aux;
			cout<<"hoja borrada";
			return;
		}
		if(anterior->izq->dato==dato){
			anterior->izq=NULL;
			delete aux;
			cout<<"hoja borrada";
			return;
